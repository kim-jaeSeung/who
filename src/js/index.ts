...
//* 경력
Promise.all([
  apiFunc<CareerData>("career"),
  apiFunc<carrerInfo[]>("careerInfo"),
]).then(([careerApiData, careerInfoApiData]) => {
  console.log("경력", careerApiData);
  console.log("경력기술서", careerInfoApiData);
  const careerData = document.getElementById("career") as HTMLDivElement;
        ...
        <div>
          ...
          <div class="careerWrap">
            <div class="commonTextWrap">
              ...
              <p> ${careerApiData.experience[0].department} ${
...
apiFunc<training>("training").then((trainingApiData) => {
  const trainingData = document.getElementById("training") as HTMLDivElement;
